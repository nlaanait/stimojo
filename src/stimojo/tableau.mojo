...
struct Tableau(Copyable, Movable):
    ...
    fn _row_to_pauli(self, half: Int, row: Int) raises -> PauliString:
        ...
        var row_w = row >> int_bit_exp
        var row_b = row & (int_bit_width - 1)
        var words = p.xz_encoding.x.n_words

        if half == 0:
            for w in range(words):
                var packed_x: Scalar[int_type] = 0
                var packed_z: Scalar[int_type] = 0

                for i in range(int_bit_width):
                    var q = w * int_bit_width + i
                    if q < self.n_qubits:
                        var x_bit = (
    ...
    fn eval(self, p: PauliString) raises -> PauliString:
        var res = PauliString.from_string("I" * self.n_qubits)
    ...
    fn apply_within(
        ...
        var inp = PauliString.from_string("I" * self.n_qubits)
