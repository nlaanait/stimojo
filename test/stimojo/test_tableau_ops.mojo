...
fn check_eval(t: Tableau, input_str: String, expected_full_str: String) raises:
    ...
    if len(p_in_temp) > 0:
        ...
        if first_char == ord("-"):
            if len(p_in_temp) > 1 and p_in_temp.as_bytes()[1] == ord("i"): # Check for "-i"
                initial_phase_val = 3
    ...
    assert_equal(String(p_out), expected_full_str) # Compare full string with phase prefix

fn check_apply_within(t: Tableau, initial_pauli_str: String, target_qubits: List[Int], expected_full_str: String) raises:
    var p_temp_str = initial_pauli_str
    var initial_phase = 0
    if len(p_temp_str) > 0:
        var first_char = p_temp_str.as_bytes()[0]
        if first_char == ord("-"):
            if len(p_temp_str) > 1 and p_temp_str.as_bytes()[1] == ord("i"): # Check for "-i"
                initial_phase = 3
    ...
    assert_equal(String(p), expected_full_str) # Compare full string with phase prefix
