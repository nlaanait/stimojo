...
def test_phase_init():
    ...
    assert_equal(p0.value, 0)
    assert_equal(p1.value, 1)
    assert_equal(p2.value, 2)
    assert_equal(p3.value, 3)

    # Modulo arithmetic in init
    var p4 = Phase(4)
    assert_equal(p4.value, 0)

    var p5 = Phase(5)
    assert_equal(p5.value, 1)
...
def test_phase_add():
    ...
    var p2 = Phase(1)  # i
    var sum = p1 + p2  # i**2
    assert_equal(sum.value, 2)
    assert_equal(String(sum), "-")

    var p3 = Phase(3)  # -i
    var sum2 = p1 + p3  # 1 + 3 = 4 -> 0 -> +
    assert_equal(sum2.value, 0)
